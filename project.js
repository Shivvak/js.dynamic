...
productList.forEach((product)=>{
  if(product.isAccessory==false){
    ...
    newimg.src= product.preview;
    newimg.alt = product.name;
    

    let newtTitle = document.createElement("h3");
    newtTitle.textContent= product.name;

    let newBrand = document.createElement("h5");
    newBrand.textContent  = product.brand;

    let newPrice = document.createElement("h4");
    newPrice.textContent =`RS ${product.price}`;
    newPrice.style.color = "#009688";


    newcard.appendChild(newimg);
    newcard.appendChild(newtTitle);
    newcard.appendChild(newBrand);
    newcard.appendChild(newPrice);

    gridB.appendChild(newcard);
